...
\on{I would say that there is no "after" until the software is dead. Development these days is understood to include the whole lifecycle, I believe.}
...
\on{I would not separate these. I agree there are two things going on here, but I like seeing them together.}
\end{itemize}
...
A key role is that of the \emph{research software engineer}\footnote{Depending on the community, other terms may be used for this role, such as ``scientific IT support staff'', ``cyberinfrastructure concierge'',  or ``research infrastructure engineer.''} (RSE), the person with expertise both in academic research, and in the software engineering tools and technology needed to build the software used in the research, and to make it sustainable. 
...
\on{I added the footnote. Is that enough? Should we mention the communities? (Scientific IT support has been used in Switzerland.) Do we really need to get into the role of faculty too?}
...
Existing work has included literature surveys of particular publication venues \todo{need references} \katznote{James is probably best to add these} \on{James -- please help!}
...
\katznote{and Geoscience Papers of the Future (GPF) -- \url{http://www.scientificpaperofthefuture.org/gpf/} (cited later as \cite{onto_soft_what_2016} though I think the URL I gave is a little better); as an older experiment - \url{http://www.runmycode.org}}
