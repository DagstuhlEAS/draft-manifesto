...
\todo{add some more specific points}
\end{abstract}
...
\on{Terms that need to be defined/introduced:\\
- Research Software Engineer\\
- intellectual contribution of software\\
...
% WAS: I will develop software in the open from the start, whenever possible.

% \on{kinda complicated. howzabout "i will choose an open source license ..." }
...
\on{"sustaining" is confusing and hard to parse. (Are we sustaining the software, or is it the software that is doing the sustaining?)}
\katznote{we want to sustain the software --- if we need to make changes to make it run, we will push those changes back for others to use (incremental updating).  If we can fix the documentation to clarify something, we will.  Etc.}
...
\on{ok?}
\katznote{I like sustainability defined as the capacity to endure.
For software, sustainability means the software will continue to be available in the future, on new platforms, meeting new needs.}
...
\on{There are still too many pledges, and several are rather confusing. Can we merge and simplify them?}

\todo{Need some intro text.
Explain what motivates these pledges.
Pledges are intended to be ``actionable.''
Why the five categories?
Explain that we first list the pledges, and then follow with detailed discussions below.}
...
\on{In the intro we need to introduce the notion of an RSE}
...
\emph{What actions are needed?} Authors need guidance about when to cite software (\ie which software should be cited) and how to cite software (\ie citation formats that will be picked up by citation tracking systems). 
...
\on{OR: When I review a publication describing a software output, I will require a link to that software so I can test it.}
\on{OR: When I review scholarly works (papers, software, \etc), I will point out missing or poorly organized software citations.} \katznote{I think this was intended to be some of both, but mostly the latter}
...
\emph{Contradictions or concerns.} The software that needs to be cited may not be published, making it hard to determine how to cite it.  There may be limits on the numbers of citations permitted by a journal.  There may be a page limit for the text version of the work.  There may not be a clearly defined way to add citations for non-text products, such as software, either in the software itself or in the published metadata (\eg in Zenodo\footnote{\url{http://zenodo.org} \on{Should these be footnotes or full bibliographic references?}} or figshare\footnote{\url{https://figshare.com}}).
...
\on{Should these be proper bibliographic references?}
}
...
\todo{Somewhere earlier in the document introduce the term ``research software engineer''}
...
Furthermore, we can provide a \meh{template language} \on{obscure. is it a template? a language? a template for language? a language for templates?} \katznote{I think this is `a template' (written in a language)} and seek to have respected organizations recommend \meh{them}. \on{what does "them" refer to?}
...
\on{needs a discussion clarifying what is meant by ``intellectual contribution''}

% ----------
\paragraph{I will publish the intellectual contributions of my research software.}


\on{Not very clear what is meant by "intellectual contributions". Can the pledge be rewritten to make this clear? Or does it go into the discussion?}
...
\on{Can this be folded into the first pledge in this section?}
...
\emph{What actions are needed?} Blogging? \chg{Upfronting}{Emphasizing} the ``forgotten'' aspects in project contribution guidelines.
...
% \on{also: I will encourage software authors to release their software without judgement. }
...
\on{a bit convoluted? Candidate for deletion or merging.}
...
% Was: "I will consider the context in which software engineering "solutions" are applicable, and acknowledge that they are substantial investments for scientists."
...
It is hard enough to get funding to support primary software development. \chg{Offerors}{Researchers?} under any sort of financial pressure may have difficulty prioritizing such support over new development and innovation. 
...
\todo{Refer to related work:
Executable papers, Peter Sloot (Elsevier?s executable paper competition),
SHARE [2.0] Pieter van Gorp}


% --------------------------------------------------
\subsection{Facilitating software discovery within and across disciplines}

\todo{Somewhat telegraphic. To be expanded.}
...
Example efforts currently include Depsy\footnote{\url{http://depsy.org}} \todo{reference (Piwowar and Priem) --- can't find this. Is it a paper?} and the Scientific Software Network Map\footnote{\url{http://scisoft-net-map.isri.cmu.edu}} \todo{(Bogart, Howison, and Herbsleb) --- is this a paper?}.
